...
use bollard::query_parameters::{BuildImageOptions, BuildImageOptionsBuilder};
...
impl Challenge {
    pub fn parse(source: String) -> Result<Challenge> {
        Ok(toml::from_str(&source)?)
    }

    pub fn parse_dir(chall_dir: PathBuf) -> Result<Challenge> {
        let file_data = fs::read_to_string(&chall_dir.join("challenge.toml")).map_err(|_| eyre!("Failed to read challenge.toml, Make sure it exists at the root of your challenge directory"))?;
        ...
        if !toml.contains_key("id") {
            let mut id_parts = chall_dir
                ...
                .ok_or_eyre("Failed to convert OsStr to Str")?;
            id_parts.reverse();
            let id = id_parts.join("/");
            toml.insert(String::from("id"), toml::Value::String(id.clone()));
        }

        let id = toml.get("id").unwrap().clone();

        Ok(toml
            .try_into()
            .map_err(|e| eyre!("failed to parse parsing {id}: {e}"))?)
    }
    ...
    pub fn get_all(root: &PathBuf) -> Result<Vec<Challenge>> {
        ...
        paths
            .into_iter()
            .map(|path| Challenge::parse_dir(path))
    ...
    pub async fn build(
        ...
        for (name, container) in &self.containers {
            ...
            let container_id = self.container_id(name);
            ...
            println!("building image: {}", container_id);
            let mut build = docker.build_image(options, None, Some(body_full(contents.into())));
            while let Some(build_step) = build.next().await {
                if let Some(stream) = build_step
                    .map_err(|err| eyre!("docker error: {err:?}"))?
                    .stream
                {
                    print!("{stream}")
                }
