...
echo "Building api..."
cargo build --bin nerine-api
echo "Building deployer server..."
cargo build --bin nerine-deployer-server
...
tail -f /dev/null | tail -f /dev/null & PID2=$!; PID1=$(jobs -p %+); exec 11>/proc/$PID1/fd/1 12</proc/$PID2/fd/0; disown $PID2; kill $PID1 $PID2

RUST_LOG=debug unbuffer ./target/debug/nerine-api >&5 2>&5 &
(cd frontend && unbuffer pnpm dev) >&7 2>&7 0</dev/null &
sudo -n nginx -c $PWD/test/nginx.conf >&9 2>&9 &
RUST_LOG=debug unbuffer ./target/debug/nerine-deployer-server >&11 2>&11 &
while read -r line; do printf "[\033[0;35mnerine-api\033[m] %s\n" "${line}"; done <&6 &
while read -r line; do printf "[\033[0;34mastro   \033[m] %s\n" "${line}"; done <&8 &
while read -r line; do printf "[\033[1;30mnginx   \033[m] %s\n" "${line}"; done <&10 &
while read -r line; do printf "[\033[0;35mnerine-dly\033[m] %s\n" "${line}"; done <&12 &
wait
