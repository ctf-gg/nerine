...
cargo build --bin sctf-deployer-server
...
RUST_LOG=debug unbuffer ./target/debug/sctf-deployer-server >&11 2>&11 &
while read -r line; do printf "[\033[0;35msctf-api\033[m] %s\n" "${line}"; done <&6 &
while read -r line; do printf "[\033[0;34mastro   \033[m] %s\n" "${line}"; done <&8 &
while read -r line; do printf "[\033[1;30mnginx   \033[m] %s\n" "${line}"; done <&10 &
while read -r line; do printf "[\033[0;35msctf-dly\033[m] %s\n" "${line}"; done <&12 &
wait
